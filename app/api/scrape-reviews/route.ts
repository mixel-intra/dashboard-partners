...
// Port 1:1 de legacy/api/scrape-reviews.js.
//
...
//
// Modos de invocación:
//   GET  /api/scrape-reviews              → cron job (vercel.json, 06:00) — todos los hoteles unlocked
//   POST /api/scrape-reviews?client=slug  → manual (botón "Ejecutar scrape ahora")
//
// Env requeridas: ADMIN_SUPABASE_URL, ADMIN_SUPABASE_SERVICE_KEY,
// BRIGHT_DATA_API_TOKEN, ANTHROPIC_API_KEY, CRON_SECRET (opcional).

// Reemplaza el bloque `functions` del vercel.json viejo (era el único endpoint
// con duración extendida — Bright Data puede tardar hasta 5 min por snapshot).
export const maxDuration = 300;

// Dataset IDs de Bright Data (Web Scraper API)
const BD_DATASETS: Record<string, string> = {
...
async function handleScrape(req: NextRequest) {
  ...
  if (!adminUrl || !adminKey) {
    return json({ error: 'ADMIN_SUPABASE_URL / ADMIN_SUPABASE_SERVICE_KEY missing' }, 500);
  }
  if (!process.env.BRIGHT_DATA_API_TOKEN) {
    return json({ error: 'BRIGHT_DATA_API_TOKEN missing' }, 500);
  }
  ...
  try {
    ...
    if (cErr) return json({ error: 'admin db: ' + cErr.message }, 500);
...
interface ScrapeSummary {
  client: string;
  sources: Record<string, { fetched: number; inserted: number }>;
  inserted: number;
  errors: { source: string; error: string }[];
}

async function scrapeClient(admin: SupabaseClient, client: any): Promise<ScrapeSummary> {
  const slug = client.id_slug;
  const cfg = client.social_listening_config || {};

  const summary: ScrapeSummary = { client: slug, sources: {}, inserted: 0, errors: [] };
  ...
  const newCfg = {
    ...
    last_scrape_error: summary.errors.length
      ? summary.errors.map((e) => `${e.source}:${e.error}`).join(' | ')
      : null,
  };
  await admin.from('clients_config').update({ social_listening_config: newCfg }).eq('id_slug', slug);
...
// ─────────────────────────────────────────────────────────────────────
async function scrapeSource(source: string, url: string): Promise<any[]> {
...
async function ingestReviews(
  ...
  const have = new Set((existing || []).map((e: any) => e.source_review_id));
...
function normalize(source: string, r: any) {
  ...
  const normalizedRating =
    source === 'booking' && rating > 5 ? Math.round((rating / 2) * 10) / 10 : isNaN(rating) ? null : rating;
...
// ─────────────────────────────────────────────────────────────────────
async function analyzeBatch(reviews: any[]): Promise<any[]> {
  const BATCH = 15;
  const out: any[] = [];
...
async function callClaude(reviews: any[]): Promise<any[]> {
