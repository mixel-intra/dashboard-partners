...
export const F = {
  ...
  telefono: (l: Lead) => firstNonEmpty(l.telefono, l.phone, l.celular) || '',
...
export function esDescartado(l: Lead) {
  ...
  return id ? id === ST.RECHAZADO : /rechaz|descart/.test(F.estado(l).toLowerCase());
}
export function esSinRespuesta(l: Lead) {
  const id = F.estadoId(l);
  return id ? id === ST.SIN_RESPUESTA : /sin.?respuesta/.test(F.estado(l).toLowerCase());
}
export function esCalificado(l: Lead) {
  const id = F.estadoId(l);
  return id
    ? id === ST.ATENCION || id === ST.SEGUIMIENTO
    : /atenci[oó]n personal|seguimiento|calific/.test(F.estado(l).toLowerCase());
}
export function esAtencion(l: Lead) {
  const id = F.estadoId(l);
  return id ? id === ST.ATENCION : /atenci[oó]n personal/.test(F.estado(l).toLowerCase());
}
